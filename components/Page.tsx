...
export const Page: React.FC<PageProps> = ({ 
  ...
  const applyAspectRatio = useCallback((frameId: string, targetVisualRatio: number) => {
    ...
    closeMenus();
  }, [config.frames, ratio, pageIndex, onUpdateFrame, closeMenus]);
  ...
  // --- Snapping Logic (Same as before) ---
  const calculateSnapTargets = (activeId: string) => {
    const xPoints = new Set([0, 50, 100]);
    const yPoints = new Set([0, 50, 100]);
    if (config.margin > 0) {
      xPoints.add(marginPctX); xPoints.add(100 - marginPctX);
      yPoints.add(marginPctY); yPoints.add(100 - marginPctY);
    }
    const neighborOffset = isRightPage ? -100 : 100;
    xPoints.add(neighborOffset); xPoints.add(neighborOffset + 100); xPoints.add(neighborOffset + 50);
    if (config.margin > 0) { xPoints.add(neighborOffset + marginPctX); xPoints.add(neighborOffset + (100 - marginPctX)); }
    config.frames.forEach(f => { if (f.id === activeId) return; xPoints.add(f.x); xPoints.add(f.x + f.width); xPoints.add(f.x + f.width / 2); yPoints.add(f.y); yPoints.add(f.y + f.height); yPoints.add(f.y + f.height / 2); });
    neighborFrames.forEach(f => { xPoints.add(f.x + neighborOffset); xPoints.add(f.x + f.width + neighborOffset); xPoints.add(f.x + f.width / 2 + neighborOffset); });
    return { x: Array.from(xPoints), y: Array.from(yPoints) };
  };

  const getSnappedPosition = (current: number, width: number, targets: number[]) => {
    const edges = [{ val: current, offset: 0 }, { val: current + width / 2, offset: width / 2 }, { val: current + width, offset: width }];
    let bestSnap = current; let minDelta = SNAP_THRESHOLD; let snapType: SnapType | null = null; let guidePos: number | null = null;
    edges.forEach(edge => {
      for (const target of targets) {
        const delta = Math.abs(edge.val - target);
        if (delta < minDelta) {
          minDelta = delta; bestSnap = target - edge.offset; guidePos = target;
          if (target === 0 || target === 100 || target === 200 || target === -100) { if ((target === 100 && !isRightPage) || (target === 0 && isRightPage)) snapType = 'spine'; else snapType = 'edge'; } 
          else if (target % 50 === 0) { snapType = 'center'; } 
          else if (Math.abs(target - marginPctX) < 0.1 || Math.abs(target - (100-marginPctX)) < 0.1 || Math.abs(target - marginPctY) < 0.1 || Math.abs(target - (100-marginPctY)) < 0.1) { snapType = 'margin'; } 
          else { snapType = 'edge'; }
        }
      }
    });
    return { snapped: bestSnap, guide: guidePos !== null && snapType ? { pos: guidePos, type: snapType } : null };
  };

  // ... (Dragging and Resizing Handlers remain same as previous version) ...
  const startDragging = (e: React.MouseEvent, frame: Frame) => {
    if (resizingFrame || e.button === 2) return;
    e.preventDefault(); e.stopPropagation(); if (onInteractionStart) onInteractionStart();
    cachedSnapTargets.current = calculateSnapTargets(frame.id);
    setDraggingFrame(frame.id); onSetActiveFrameId(frame.id);
    dragStartRef.current = { x: e.clientX, y: e.clientY, initialX: frame.x, initialY: frame.y, initialW: frame.width, initialH: frame.height, initialRatio: frame.width / frame.height };
  };

  const startResizing = (e: React.MouseEvent, frame: Frame, handle: ResizeHandle) => {
    e.preventDefault(); e.stopPropagation(); if (onInteractionStart) onInteractionStart();
    cachedSnapTargets.current = calculateSnapTargets(frame.id);
    setResizingFrame({ id: frame.id, handle }); onSetActiveFrameId(frame.id);
    dragStartRef.current = { x: e.clientX, y: e.clientY, initialX: frame.x, initialY: frame.y, initialW: frame.width, initialH: frame.height, initialRatio: frame.width / frame.height };
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (draggingFrame) {
        ...
        let rawX = initialX + dx; let rawY = initialY + dy;
        ...
        let finalX = snapX.snapped; let finalY = snapY.snapped;
        if (isRightPage) { finalX = Math.max(-100, Math.min(105 - (initialW/2), finalX)); } else { finalX = Math.max(-5, Math.min(205 - (initialW/2), finalX)); }
        finalY = Math.max(-5, Math.min(105 - (initialH/2), finalY));
        setActiveGuides({ x: snapX.guide ? [snapX.guide] : [], y: snapY.guide ? [snapY.guide] : [] });
      ...
      if (resizingFrame) {
        ...
        if (!frame) return;
        let guidesX: { pos: number, type: SnapType }[] = []; let guidesY: { pos: number, type: SnapType }[] = [];
        ...
        if (frame.isLocked) {
             ...
             if (handle === 'n' || handle === 's') driver = 'height'; else if (handle === 'e' || handle === 'w') driver = 'width';
             if (driver === 'width') {
                 let targetEdge = handle.includes('w') ? rawX : (rawX + rawW);
                 let snappedEdge = targetEdge; let snapType: SnapType | null = null;
                 cachedSnapTargets.current.x.forEach(t => { if (Math.abs(t - targetEdge) < SNAP_THRESHOLD) { snappedEdge = t; snapType = (t === 0 || t === 100 || t===200) ? 'edge' : (t % 50 === 0) ? 'center' : 'margin'; if ((t===100 && !isRightPage) || (t===0 && isRightPage)) snapType = 'spine'; } });
                 if (handle.includes('w')) { finalW = (initialX + initialW) - snappedEdge; finalX = snappedEdge; } else { finalW = snappedEdge - initialX; }
                 if (snapType) guidesX.push({ pos: snappedEdge, type: snapType });
                 finalH = finalW / initialRatio; if (handle.includes('n')) finalY = (initialY + initialH) - finalH;
             } else {
                 let targetEdge = handle.includes('n') ? rawY : (rawY + rawH);
                 let snappedEdge = targetEdge; let snapType: SnapType | null = null;
                 cachedSnapTargets.current.y.forEach(t => { if (Math.abs(t - targetEdge) < SNAP_THRESHOLD) { snappedEdge = t; snapType = (t === 0 || t === 100) ? 'edge' : (t === 50) ? 'center' : 'margin'; } });
                 if (handle.includes('n')) { finalH = (initialY + initialH) - snappedEdge; finalY = snappedEdge; } else { finalH = snappedEdge - initialY; }
                 if (snapType) guidesY.push({ pos: snappedEdge, type: snapType });
                 finalW = finalH * initialRatio; if (handle.includes('w')) finalX = (initialX + initialW) - finalW;
             }
        } else {
             if (handle.includes('e') || handle.includes('w')) {
                const movingLeft = handle.includes('w'); const targetEdge = movingLeft ? rawX : (rawX + rawW); let snappedEdge = targetEdge; let snapType: SnapType | null = null;
                cachedSnapTargets.current.x.forEach(t => { if (Math.abs(t - targetEdge) < SNAP_THRESHOLD) { snappedEdge = t; snapType = (t===100 || t===0) ? 'edge' : 'margin'; if(t===100 && !isRightPage) snapType='spine'; } });
                if (snapType) { if (movingLeft) { finalX = snappedEdge; finalW = (initialX + initialW) - snappedEdge; } else { finalW = snappedEdge - initialX; } guidesX.push({ pos: snappedEdge, type: snapType }); }
             }
             if (handle.includes('n') || handle.includes('s')) {
                const movingTop = handle.includes('n'); const targetEdge = movingTop ? rawY : (rawY + rawH); let snappedEdge = targetEdge; let snapType: SnapType | null = null;
                cachedSnapTargets.current.y.forEach(t => { if (Math.abs(t - targetEdge) < SNAP_THRESHOLD) { snappedEdge = t; snapType = (t===100 || t===0) ? 'edge' : 'margin'; } });
                if (snapType) { if (movingTop) { finalY = snappedEdge; finalH = (initialY + initialH) - snappedEdge; } else { finalH = snappedEdge - initialY; } guidesY.push({ pos: snappedEdge, type: snapType }); }
             }
        }
        if (finalW < 2) finalW = 2; if (finalH < 2) finalH = 2;
        setActiveGuides({ x: guidesX, y: guidesY });
        onUpdateFrame(pageIndex, resizingFrame.id, { x: finalX, y: finalY, width: finalW, height: finalH });
      }
    };
    const handleMouseUp = () => { setDraggingFrame(null); setResizingFrame(null); setActiveGuides({ x: [], y: [] }); dragStartRef.current = null; };
    if (draggingFrame || resizingFrame) { window.addEventListener('mousemove', handleMouseMove); window.addEventListener('mouseup', handleMouseUp); }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full overflow-visible">
        ...
        {!isRightPage && <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-black/[0.03] to-transparent z-10 pointer-events-none" />}
        {isRightPage && <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-black/[0.03] to-transparent z-10 pointer-events-none" />}
      </div>
    </div>
  );
};
