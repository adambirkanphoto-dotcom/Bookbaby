...
const App: React.FC = () => {
  ...
  const performExport = useCallback(async () => {
    setIsExporting(true);
    const fileName = `bookbaby-export-${new Date().getTime()}`;
    ...
    if (exportFormat === 'JPG') {
      ...
      const renderPageOnCanvas = async (pIdx: number, offsetX: number) => {
        ...
        for (let s = 0; s < slotsCount; s++) {
          ...
          if (imgData && placedStatus[imgData.id]) {
            ...
            const y = padding + (s > 1 ? slotH + padding : 0);
            ctx.save();
      ...
      await renderPageOnCanvas(0, 0);
      if (pageConfigs.length > 1) await renderPageOnCanvas(1, pageWidth);
      canvas.toBlob((blob) => {
        if (blob) {
          const url = URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url; link.download = `${fileName}.jpg`; link.click();
        }
        setIsExporting(false); setShowExportMenu(false);
      }, 'image/jpeg', 0.92);
    } else {
      let html = `<!DOCTYPE html><html><head><title>${fileName}</title><style>@page { size: auto; margin: 0; } body { margin: 0; background: #eee; font-family: system-ui; } .spread { display: flex; width: 100vw; height: ${100 / currentRatio}vw; background: white; page-break-after: always; position: relative; } .page { flex: 1; position: relative; border: 1px solid #f0f0f0; box-sizing: border-box; display: flex; align-items: stretch; } .grid { display: grid; width: 100%; height: 100%; gap: 10px; padding: 20px; box-sizing: border-box; } .slot { background: #fafafa; position: relative; overflow: hidden; display: flex; align-items: center; justify-content: center; border: 1px solid #eee; } img { width: 100%; height: 100%; object-fit: cover; } .fit { object-fit: contain; } .meta { position: absolute; bottom: 5px; right: 5px; font-size: 8px; color: #ccc; } @media print { body { background: white; } .meta { display: none; } }</style></head><body>`;
      let imgCursor = 0;
      for (let i = 0; i < pageConfigs.length; i += 2) {
        ...
        const renderHtmlPage = (pIdx: number) => {
          ...
          const gridStyle = config.layout === 'grid-4' ? 'grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr;' : config.layout === 'double' ? 'grid-template-columns: 1fr 1fr;' : 'grid-template-columns: 1fr;';
          ...
          for (let s = 0; s < slots; s++) {
            ...
            if (img && placedStatus[img.id]) {
              ...
              pageHtml += `<div class="slot"><img src="${img.url}" class="${config.cropType}" style="transform: scale(${scale})"></div>`;
            } else { pageHtml += `<div class="slot" style="background: #f0f0f0"></div>`; }
          }
      ...
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url; link.download = `${fileName}-printable.html`; link.click();
      setIsExporting(false); setShowExportMenu(false);
    }
          ...
          <div className="relative" ref={exportMenuRef}>
            ...
            {showExportMenu && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-[#1e1e1e] border border-[#444] rounded-lg shadow-2xl p-4 z-[100] flex flex-col gap-4">
                ...
                >
                  {isExporting ? 'Processing...' : `Download ${exportFormat}`}
                </button>
              </div>
            )}
          </div>
        ...
        <main className="flex-1 overflow-y-auto bg-[#181818] hide-scrollbar scroll-smooth relative">
          <div className="py-24 px-12 flex flex-wrap justify-center gap-x-12 gap-y-40 min-h-full relative z-10">
            {spreads.map((spread, sIdx) => (
              <div key={sIdx} className="group relative flex flex-col items-center">
